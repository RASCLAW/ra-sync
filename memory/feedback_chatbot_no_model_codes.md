...
- `get_catalog_text()` in `chatbot/knowledge_base.py` deliberately omits the `code` field from its system-prompt output (the dict still has `code` for internal reference only).
