...
**Why:** Session 99 almost committed 3 chatbot/ files (conversation_engine.py, knowledge_base.py, security.py) plus 2 deleted files -- all active work-in-progress in RA's other IDE tab. Three risks of committing another session's work:
...
4. Watch for "file just opened in IDE" context hints -- that's usually the other session's focus area (e.g. session 99 RA opened chatbot/security.py in the IDE, confirming that was the other session's work)
