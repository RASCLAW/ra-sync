...
- **Rename**: `chatbot/` → `chatbot/` (via `git mv`). Task Scheduler re-registered, 8 path refs updated, log renamed to `.tmp/chatbot-server.log`. Some memory files still reference `chatbot/` — catch on next `/lint-memory` sweep.
